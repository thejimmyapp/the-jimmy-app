...
export function AppShell({ rail, stage, dock, className = "", boardTheme, pieceStyle, pieceSize }: AppShellProps) {
  return (
    <main className={`app-shell app-shell-rail-stage-dock ${className}`} data-board-theme={boardTheme} data-piece-style={pieceStyle} data-piece-size={pieceSize}>
      ...
      <aside className="app-rail" aria-label="Application navigation">{rail}</aside>
      ...
      {dock && <aside className="app-dock" aria-label="Task tools"><div className="app-dock-content">{dock}</div><div id="app-dock-panel" /></aside>}
    </main>
  );
}
